...
**Canonical thresholds are in `references/computation-formulas.md`.** The values below are a readable summary — if they ever diverge, computation-formulas.md wins.
...
Apply the **Guardrail Zones** section from `references/computation-formulas.md` exactly. Use the **Zone Assignment** algorithm: compare each metric to thresholds in order from most severe (CRITICAL first).

Every guardrail gets exactly one zone. Never skip the assignment step.
