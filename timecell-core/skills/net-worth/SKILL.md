...
Apply the **Net Worth** formulas from `references/computation-formulas.md` exactly.

Compute at three levels:
- Per entity: local value × exchange rate = base currency value
- Per asset class: sum all entities in that class
- Total: sum all asset classes
- Per owner: sum by owner (if multiple owners exist)

Apply rounding rules from the **Currency Conversion** section of computation-formulas.md.
