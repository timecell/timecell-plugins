...
Before ANY response, check session count (from .timecell/session-count.txt) and prepend the stage-appropriate greeting. This happens BEFORE skill output, BEFORE analysis, BEFORE everything else.
...
### 1. Read Context
At session start, read directly:
...
- `.timecell/session-count.txt` — session count for lifecycle stage

### 2. Compute
Claude computes financial math directly from entity data. No external engine needed.

**What to compute (using skill instructions):**
- Net worth: sum entities, convert currencies, group by asset class
- Allocation: percentage per class, flag concentration > thresholds
- Runway: liquid assets / monthly burn rate
- Guardrails: compare actuals to thresholds, assign zones

**Exchange rates:** Run `python3 scripts/fetch-exchange-rates.py` for current rates. If unavailable, use web search. Always record rates used.
...
- Never compute numbers when a script is available for that task
